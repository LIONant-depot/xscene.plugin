#ifndef XSCENE_COMMANDS_ENTITY_LIFECYCLE_H
#define XSCENE_COMMANDS_ENTITY_LIFECYCLE_H
#pragma once

// Create/Delete Entity - phase 4 of the command/undo plan. Replaces
// the Level tree panel's direct calls (ShowCreateMenuItems' "New Entity", DoDeleteEntity) with
// real xundo commands.
//
...
// add_component_cmd::Undo (xscene_commands_component_edit.h) doesn't special-case anything either: it
...
namespace xscene::commands
{
    ...
    // Mirrors the Level tree panel's own DoDeleteEntity exactly, including the selection/
    ...
    // BackupCurrenState/Undo for m_lComponents + m_HierarchyDiffs (xscene_commands_make_prefab.h).
    ...
    // (moved verbatim, no behavior change) so make_prefab_cmd (xscene_commands_make_prefab.h) can reuse
    ...
    inline void SnapshotSubtreeForRestore(scene_context& Ed, xundo::undo_file& File, xecs::scene::guid SceneGuid, xecs::scene::permanent_id RootId) noexcept
    {
        ...
        {
            const auto FolderId = xscene::FindFolderContaining(*pScene, RootId);
        ...
        // part of this at all. DeleteEntitySubtree's own top-level step (xscene_prefab_authoring.h) scrubs
        ...
        SnapshotContainingPrefabOverrides(Ed, File, SceneGuid, RootId);
    }
    ...
    inline void RestoreSubtreeFromSnapshot(scene_context& Ed, xundo::undo_file& File, xecs::scene::guid SceneGuid, xecs::scene::permanent_id RootId) noexcept
    {
        ...
        if (Entries.empty())
        {
            ...
            RestoreContainingPrefabOverrides(Ed, File, SceneGuid);
            return;
        }
        ...
        {
            xscene::ReparentEntityIntoFolder(*pScene, RootId, static_cast<xecs::scene::folder_id>(FolderVal));
        ...
        // Reverse DeleteEntitySubtree's own top-level side effect (xscene_prefab_authoring.h): it scrubbed
            ...
            if (auto ParentIt = pScene->m_LocalToRuntime.find(static_cast<xecs::scene::permanent_id>(RootParentId)); ParentIt != pScene->m_LocalToRuntime.end())
            {
                ...
                if (PDetails.m_pPool && PDetails.m_pPool->m_pArchetype->getComponentBits().getBit(xecs::component::type::info_v<xecs::component::children>.m_BitID))
                {
                    if (auto RootIt = pScene->m_LocalToRuntime.find(RootId); RootIt != pScene->m_LocalToRuntime.end())
                    {
                        ...
                        Ed.World().m_SceneMgr.MarkEntityDirty(SceneGuid, static_cast<xecs::scene::permanent_id>(RootParentId));
                    }
                }
            }
        }
    ...
    //================================================================================================
    struct create_entity_cmd : scene_command
    {
        ...
        void BackupCurrenState(xundo::undo_file& File) noexcept override
        {
            ...
            // (the command/undo known-gaps list's own gap #2). Recorded here, BEFORE Redo runs, since
        ...
        void Undo(xundo::undo_file& File) noexcept override
        {
            ...
            // Gap #2 fix (the command/undo known-gaps list): DeleteSubtreeByPermanentId above already
    ...
    // snapshot/restore machinery both this command and make_prefab_cmd (xscene_commands_make_prefab.h)
    // build on.
    //================================================================================================
    struct delete_entity_cmd : scene_command
    {
        delete_entity_cmd(xundo::system& System, void* pDataBase) noexcept : scene_command(System, "DeleteEntity", pDataBase) { RegisterArguments(); }
        const char* getCommandHelp() const noexcept override
        {
        ...
        // BackupCurrenState first - see set_property_cmd's own comment, xscene_commands_property_edit.h,
...
#endif // XSCENE_COMMANDS_ENTITY_LIFECYCLE_H
