#ifndef XSCENE_COMMANDS_PROPERTY_EDIT_H
#define XSCENE_COMMANDS_PROPERTY_EDIT_H
#pragma once

// Property editing - phase 2 of the command/undo plan. Replaces
// entity_inspector_bridge::m_OnPropertyChanged's own DIRECT, unconditional override-recording
// (xscene_entity_inspector_bridge.h - "every commit... writes only the new value... there is no automatic
...
namespace xscene::commands
{
    ...
    // pool-lookup pattern exactly (xscene_entity_inspector_bridge.h) - re-derived fresh every call rather
    ...
    inline void RecordPropertyOverride(scene_context& Ed, const resolved_property_target& Target, xecs::scene::guid SceneGuid, xecs::scene::permanent_id Id, const std::string& Path, const std::string& ValueStr) noexcept
    {
        ...
        auto Ctx = xscene::FindContainingPrefabInstance(Ed.World(), Target.m_Entity);
    ...
    // (xscene_entity_inspector_bridge.h, m_OnOverrideReset): erase the matching property override, and if the
    ...
    // Wired from entity_inspector_bridge::m_OnOverrideReset (xscene_entity_inspector_bridge.h) - the old
    // inline BeginEdit/setProperty/erase_if path bypassed the command bus (no Ctrl+Z).
    //================================================================================================
    struct revert_override_cmd : scene_command
    {
        revert_override_cmd(xundo::system& System, void* pDataBase) noexcept : scene_command(System, "RevertOverride", pDataBase) { RegisterArguments(); }
        const char* getCommandHelp() const noexcept override
        {
