#ifndef XSCENE_COMMANDS_COMPONENT_EDIT_H
#define XSCENE_COMMANDS_COMPONENT_EDIT_H
#pragma once

// Add/Remove Component - phase 3 of the command/undo plan. Replaces
// xscene_panel_entity_properties.h's own DIRECT calls to GameMgr.AddOrRemoveComponents (the "Add
// Component" combo and the component header's "[X]") with real xundo commands, same pattern as
// phase 2's SetProperty (xscene_commands_property_edit.h, included below for
...
// in-place mutation) - every call site in this codebase (xscene_panel_entity_properties.h,
// xscene_prefab_authoring.h, xscene_prefab_overrides.h) manually remaps the owning scene's
...
// xscene_entity_inspector_bridge.h) and records a {Path, TypeGuid, ValueStr} list - the same string-based
...
// restored on Remove/Undo - the command/undo known-gaps list's own recorded gap #4: the original
...
// RemovePropertyOverride, xscene_commands_property_edit.h, already uses) right after the component
...
namespace xscene::commands
{
    ...
    // be present - unlike ResolvePropertyTarget (xscene_commands_property_edit.h), which deliberately
    ...
    // site already does by hand (xscene_panel_entity_properties.h lines 65-71/148-154): erase the old
    ...
    // replays every value onto Target via SetLivePropertyValue (xscene_commands_property_edit.h). Called
    ...
    // RemovePropertyOverride (xscene_commands_property_edit.h) already uses for a single property, just at
    ...
    // RemovePropertyOverride's own identical check, xscene_commands_property_edit.h) - m_lComponents lives
    ...
    // FindOrCreateOverrideEntry (xscene_prefab_overrides.h), same helper the live "edit a property"
    ...
    inline void RestoreComponentOverrideEntry(scene_context& Ed, xundo::undo_file& File, xecs::scene::guid SceneGuid, xecs::component::entity Entity, std::uint64_t ComponentTypeGuidValue) noexcept
    {
        ...
        auto& CompOverride = xscene::FindOrCreateOverrideEntry(*Ctx.m_pPI, ComponentTypeGuidValue, MemberPath);
        for (auto& O : Overrides)
    ...
    struct remove_component_cmd : scene_command
    {
        ...
        std::string Redo() noexcept override
        {
            ...
            // Scrub the now-meaningless override entry (gap #4, the command/undo known-gaps list) -
...
#endif // XSCENE_COMMANDS_COMPONENT_EDIT_H
